# Table of Contents

* [Nabeel's Docs](README.md)

## Blockchain 

* [Hyperledger Composer Basics](blockchain/composer.md)
* [Configure Blockchain on Windows](blockchain/config-windows.md)
* [Essentials](blockchain/essentials.md)
* [Working with Fabric - Part 1](blockchain/fabric-via-docs-1.md)
* [Working with Fabric - Part 2](blockchain/fabric-via-docs-2.md)
* [Foundation](blockchain/foundation.md)
* [IBM Cloud Blockchain VSCode Extension](blockchain/ibmblockchainvscode.md)

## C++ 

* [Introduction to C++](c++/introduction.md)

## Chef 

* [Basics of Chef](chef/basics.md)
* [Manage a Node with Chef Server](chef/manage-node-with-server.md)

## Cloud Application Developer 

* [DevOps](cloud-application-developer/devops.md)

## Containers And Microservices 

* [Build an Express Application that uses MongoDB](containers-and-microservices/build-an-express-app-with-mongo.md)
* [Containers and Kubernetes on IBM Cloud](containers-and-microservices/containers-and-kubernetes-on-IBM-cloud.md)
* [Deploy an Express Application that uses MongoDB on k8s Locally](containers-and-microservices/deploy-an-express-app-with-mongo-on-k8s-locally.md)
* [Docker Configuration Scripts](containers-and-microservices/docker-config.md)
* [Multi Stage Docker Builds](containers-and-microservices/docker-multi-stage.md)
* [Docker](containers-and-microservices/docker.md)
* [Infrastructure and Container Management](containers-and-microservices/infrastructure-and-container-management.md)
* [Istio with Kubernetes](containers-and-microservices/istio-with-kubernetes.md)
* [Microservices with Istio](containers-and-microservices/microservices-with-istio.md)

## Data Science 

* [Introduction](data-science/introduction.md)
* [Methodology](data-science/methodology.md)
* [Data Science Tools](data-science/tools.md)

## Design Patterns 

* [OOP Review](design-patterns/introduction.md)
* [Observer Design Pattern](design-patterns/observer.md)
* [Strategy Design Pattern](design-patterns/strategy.md)

## Dotnet 

* [CASP.NET Boilerplate Setup](dotnet/abp-setup.md)
* [Introduction to C](dotnet/intro-to-cs.md)
* [Introduction to F](dotnet/intro-to-fs.md)

## Dsx With Python 

* [Data Analysis With Python](dsx-with-python/data-analysis.md)
* [Deep Learning Fundamentals](dsx-with-python/intro-to-deep-learning.md)
* [Machine Learning with Python](dsx-with-python/machine-learning.md)
* [Python Basics](dsx-with-python/python-basics.md)

## Dsx With R 

* [Inference & Modeling](dsx-with-r/inference-and-modeling.md)
* [Probability](dsx-with-r/probability.md)
* [R Basics](dsx-with-r/rbasics.md)
* [Visualization](dsx-with-r/visualization.md)

## Gitlab 

* [CI Pipelines](gitlab/ci-pipelines.md)

## Go 

* [Intro to Go](go/basics.md)

## Ibm Cloud 

* [Calling External API's from Watson Assistant](ibm-cloud/calling-external-apis-from-watson-assistant.md)
* [Deploy a Kubernetes App on ICP](ibm-cloud/deploy-a-kubernetes-app-on-icp.md)
* [Deploy a Python Machine Learning Model as a CF Web Service](ibm-cloud/deploy-a-python-machine-learning-model-to-cf.md)
* [Deploy Angular App to CF with DevOps](ibm-cloud/deploy-angular-app-to-cf.md)
* [Enale Cors for Node-RED](ibm-cloud/enable-cors-for-node-red.md)
* [Publish an App to CF](ibm-cloud/publishingappcf.md)
* [Setting up a DSX Notebook](ibm-cloud/setting-up-a-dsx-notebook.md)
* [Watson API's and ML](ibm-cloud/watson-apis-and-ml.md)
* [Watson IoT Messaging - Node](ibm-cloud/watson-iot-messaging.md)

## Ionic 

* [Solving the HTTP CORS Issue with Ionic Applications](ionic/solving-http-cors.md)

## Iot 

* [MQTT Messaging with Mosquitto](iot/mqtt-message-broker-with-mosquitto.md)

## Javascript 

* [Web Components](javascript/web-components.md)
* [Real Time Communication with WebRTC](javascript/web-rtc.md)

## Linux 

* [Configure Jenkins](linux/configure-jenkins.md)
* [Server Setup](linux/setup-ubuntu-server.md)

## Nodejs 

* [Handling Cors](nodejs/cors.md)
* [Startover NPM Package](nodejs/creating-a-file-watching-application.md)
* [Express Application with Typescript](nodejs/express-application-with-typescript.md)
* [Introduction to SQLite with Node.js](nodejs/intro-to-sqlite.md)
* [Basic Modules](nodejs/module1.md)
* [Play audio from a Javascript XHR Request](nodejs/play-audio-from-xhr.md)
* [Static Generator and Compilers](nodejs/static-and-compiles.md)
* [Node-gyp](nodejs/troubleshooting.md)

## Presentations 

* [Jasmine and Karma](presentations/jasmineandkarma.md)

## Random 

* [Deployment Platforms](random/deployment.md)
* [Frameworks and App Platforms](random/frameworks.md)
* [Powershell](random/powershell.md)
* [VirtualBox Bugs](random/virtuabox.md)

## React 

* [React with Redux](react/complete-react.md)
* [Module 1 - JSX and React Components](react/module-1.md)
* [Module 2 - State, Lifecycle and Event Handlers](react/module-2.md)
* [Module 3 - Lists and Forms](react/module-3.md)
* [#Generating a PWA for a React App](react/pwa.md)

## Slack 

* [Hello World App](slack/hello-world-app.md)

## Tj Bot 

* [Setting up a Bot](tj-bot/setting-up-a-bot.md)


