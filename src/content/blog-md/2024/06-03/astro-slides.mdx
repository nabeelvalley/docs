...
::view-transition-old(next) {
  animation: slide-out-next 0.5s linear;
}
::view-transition-new(next) {
  animation: slide-in-next 0.5s linear;
}
```

</Slide>

In the above we set that the `presentation-next` class defines a `view-transition-name` called `next`. Then, we define the transitions for the `old` and `new` content that applies to the `next` transition name as an Animation. We are referencing a `slide-in-next` and `slide-out-next` animation which are define as follows:
...
Lastly, the implementation for the `presentation-prev` transition is pretty similar just with lots of numbers flipped around:
...
::view-transition-old(prev) {
  animation: slide-out-prev 0.5s linear;
}
::view-transition-new(prev) {
  animation: slide-in-prev 0.5s linear;
}
