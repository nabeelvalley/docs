...
The HTML in the above example is rendered within the canvas and is - very importantly - exposed to the accessibility tree
...
The example above already suffers from the fact that the size of the canvas content doesn't match the canvas and so things are not quite in the right place. [Matt Rothenberg](https://mattrothenberg.com/) also points this out in their [HTML in Canvas](https://mattrothenberg.com/notes/html-in-canvas/) post and while it's definitely doable it does require a fair amount of wrangling to make work nicely

The aforementioned sizing problem is probably relatively easy to abstract neatly which is worth considering if/once the API is standardized but I don't really want to get into that effort just yet
...
The code for the above example can be seen below:

```html
<site-shader-canvas>
  <canvas width="1000" height="500" style="width: 100%" layoutsubtree>
    <div class="html-in-canvas" style="color: green; display: flex; flex-direction: column; align-items: center; justify-content: center;">
