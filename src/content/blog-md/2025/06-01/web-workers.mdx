...
We can see that we have a `fun` function that is called when we receive the `message` as before, we also have a type specified for the data we expect to receive
...
<Snippet path="web-workers/vite/src/canvas.ts" />
