...
You may also want to support alias resolution, in that case you can provide `tsconfig-paths` as well as the path to the config file that defines them if non-default:
...
const worker = new Worker("my-worker.ts", {
    ...
    env: {
      ...
      TS_NODE_PROJECT: "tsconfig.project.json"
  })
